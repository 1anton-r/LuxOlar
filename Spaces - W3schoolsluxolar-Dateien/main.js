/* Iframe wrapper JS used on the client side */
  ...
  window.addEventListener('message', message => {
    ...
    if (message.data !== undefined && message.data.action !== undefined) {
      ...
      if (message.data.action === 'LAYOUT' || message.data.action === 'INIT') {
        ...
        // let iframeStdHeight = String(message.data.mainNavHeightPx + message.data.secondaryNavHeightPx) + 'px';
        ...
        if (message.data.expanded) {
          ...
          // unset back to css value
          iframeElm.style.height = '';
          // iframeElm.style.height = iframeStdHeight;
        }
      }

      if (message.data.action === 'REDIRECT') {
        let destUrl = message.data.url;

        if (destUrl.indexOf('%ENCODED_ORIGIN_URL%') !== 0) {
          // replace placeholder with origin url
          destUrl = destUrl.replace('%ENCODED_ORIGIN_URL%', encodeURIComponent(window.location.href));
        }
